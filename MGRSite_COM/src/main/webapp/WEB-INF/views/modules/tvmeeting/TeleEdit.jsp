...
<html>
  ...
  <ul class="nav nav-tabs">
		...
		<li class="active"><a href="javaSctipr:void(0)">修改会商</a></li>
		</li>
	</ul>
	<input id="chk_value" name="chk_value" type="hidden"
		value="${chk_value}" />
	<tags:message content="${message}"/>
    <form:form id="inputForm" name="inputForm" modelAttribute="tele" action="${ctx}/tvmeeting/meetingarrange/updateSave" method="post" class="form-horizontal" onsubmit="return check()">
  		<form:hidden path="MeetingId" />
  		<div class="control-group">
			<label class="control-label">日期:</label>
			<div class="controls">
				<input type="text" class="Wdate" id="MeetingDate" name="MeetingDate" placeholder="请选择日期" value="${tele.meetingDate}" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>			
			</div>
		</div>
	    <div class="control-group">
	   		 <label style="margin-left: 130px;">开始时间:</label>
				<div style="display: inline;width:30px;">
					<input type="text" class="Wdate" id="BeginTime" name="BeginTime" style="width: 60px;" value="${tele.beginTime}" onfocus="WdatePicker({dateFmt:'HH:mm',minDate:'8:00',maxDate:'18:00'})"/>
				</div>
			结束时间:
				<div style="display: inline;">
					<input type="text" class="Wdate" id="EndTime" name="EndTime" style="width: 60px;" value="${tele.endTime}" onfocus="WdatePicker({dateFmt:'HH:mm',minDate:'8:00',maxDate:'18:00'})"/>
				</div>
			<span id="msg"></span>
		</div>
		<div class="control-group">
...
<!-- 				<table id="attachList"  style="width: auto;" border="1">				 -->
		...
		<%int i=1; %>	
		<thead style='text-align:center;'><th>序号</th><th>文件名</th><th>操作</th></thead><tbody>
		<c:forEach items="${list }" var="attach">
					<tr id="${attach.id}">
						<td style="width: auto;"><%=i++ %></td>
						<td style="width: auto;">${attach.docName}</td>						
						<td style="width: auto;">
<%-- 						<a href="${ctx}/notice/attach/delById?ID=${attach.ID}&kind=${kind}&subKind=${subKind}&linkURL=${attach.linkURL}&noticeID=${attach.noticeID }">删除</a> --%>
						<a href='javascript:void(0)' onclick='delAttach(\""+attach.id+"\",\""+(attach.docUrl)+"\",\""+(attach.docId)+"\")'>删除</a>
						</td>
					</tr>
				</c:forEach>
				</table>			
		...
		<div class="control-group" id="upload">
			...
			<div class="controls">
				...
				<input type="button" value="上传" onclick="return checkupload()" >
				<span style="color:red;">上传文件大小不得超过${ maxUploadSize}M</span>
				<span id="docurl"></span>
			</div>	 
		</div>
		<div class="control-group">
