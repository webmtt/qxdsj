...
$(function(){
	$("#dataId").blur(function(){
		...
		delAttach();
		if (dataId=="") {
			$("#checkName").html("数据ID不能为空！");
			return false;  
		}else{
			if(status==1){
				$("#checkName").html("数据ID已经存在！");
				return false;  
			}else{
				$("#checkName").empty();
				return true;  
...
<body>
	<ul class="nav nav-tabs">
		<li ><a href="${ctx}/dt/distribute/list">分发定义列表</a></li>
		 <li class="active"><a href="${ctx}/dt/distribute/add">新增分发类型</a></li> 
	</ul>
<tags:message content="${message}"/>
 <form:form id="inputForm" name="inputForm" modelAttribute="ddsDataDef" action="${ctx}/dt/distribute/SaveDdsDataDef" method="post" class="form-horizontal" onsubmit="return check()">
		<form:input path="SearchType" type="hidden" value="0" />
		<div class="control-group">
			<label class="control-label">数据ID:</label>
			<div class="controls">
			    <form:input path="dataId" htmlEscape="false" maxlength="50"
		...
		<div class="control-group">
			...
			<div class="controls">
			   <input type="radio" id="interfaceType1" name="interfaceType" value="MUSIC" checked="checked" />		
				MUSIC接口
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<input type="radio" id="dataFormat1" name="dataFormat" value="txt" checked="checked" />		
				支持txt
				<input type="radio" id="dataFormat2" name="dataFormat" value="csv" checked="checked"/>
				支持csv
			</div>
		</div>
		<div class="control-group">
