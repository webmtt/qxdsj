...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="javascript:void(0)">${not empty dataCategory.categoryid?'编辑':'新增'}资料种类</a></li>
	</ul>
	<form:form id="inputForm" modelAttribute="dataCategoryDef" action="${ctx}/dataService/saveDataCategory" method="post" enctype="multipart/form-data" class="form-horizontal" >
...
<%--			         <input type="radio" value="0" name="zllx" <c:if test="${type==0}"> checked="checked" </c:if>/>资料大类--%>
<%--			         <input type="radio" value="1" name="zllx"  <c:if test="${type==1}">checked="checked" </c:if>/>资料子类--%>
<%--			</div>--%>
		<div class="BIGTYPE0">
			<div class="control-group">
