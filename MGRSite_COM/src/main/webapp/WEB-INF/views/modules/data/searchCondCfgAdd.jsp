...
.conTemp1,.conTemp2,.conTemp3,.conTemp4{float:left;margin-top:10px;}
...
function isvalueLimit(){
	var value= $("input[name='isvalueLimit']:checked").val();
	if(value=="1"){
		 $(".limitShow").show();
	 }else{
		 $(".limitShow").hide();
...
function endDateLimit(){
	var value= $("input[name='endDateType']:checked").val();
	if(value=="0"){
		 $("#endDateStatus").hide();
	 }else{
		 $("#endDateStatus").show();
...
function saveBottom(){
	...
	$.ajax({
		url : "${ctx}/dataSearchDef/saveSearchSetDef",
		type : "post",
		dataType : "json",
		async : true,
		...
		success : function(result) {
			...
			//$("#myModal", parent.document).modal({show:false});
		}
	});
}
function closeWin(){
...
<body>
	...
	<form id="inputForm"  action="${ctx}/dataSearchDef/saveSearchSetDef" method="post" enctype="multipart/form-data" class="form-horizontal" >
	<input type="hidden" value="${groupcode}" id="groupcode" name="groupcode"/>
	<input type="hidden" value="${searchgroupcode}" id="searchgroupcode" name="searchgroupcode"/>
	<c:if test="${groupcode=='TimeSel'}">
          <div class="TimeSel">
	          ...
	          <div class="isValueLimit">
	             ...
	             <div class="limitShow">
		             <label class="control-label">检索时间限制：</label>
		             <input type="text" name="valueLimit" style="width:50px;margin-top: 10px;" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
	          ...
	          <div class="valueRange">
	              ...
	              <input type="text" name="minValue" style="width:50px;margin-top: 10px;" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/> ~ <input type="text" name="maxValue" style="width:50px;margin-top: 10px;" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
	          </div>
	          ...
	          <div class="defaultValue">
	              ...
	              <div class="conTemp1">
		               ...
		               <input type="text" name="startTime" style="width:40px;margin-top: 10px;float:left;" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/> 
		               <label style="float:left;margin-top:15px;margin-left:10px;">结束时间：</label>
		               <div class="defaultText">当日-</div>
		               <input type="text" name="endTime" style="width:40px;margin-top: 10px;float:left;"/>
	              </div>
	              <div class="conTemp2">
	                   <!-- yyyyMMXU当前月（两种情况）-->
	                   <p style="color:red;text-align: center;">默认日期（开始结束）时间</p>
	                   ...
	                   <select id="xunChose" name="xunChose" style="float:left;">
	                        <option value="SEQ">上旬/中旬／下旬表示“01/02/03”</option>
	                        <option value="DIV">上旬/中旬／下旬表示“01/11/21”</option>
		               </select>
		               <br/>
	                   <label class="control-label">开始时间：</label>
	                   <div class="currentMonth">当月-</div>
		               <input type="text" name="startTime2" style="width:40px;margin-top: 10px;float:left;"  onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/> 
		               ...
		               <div class="currentMonth">当月-</div>
		               <input type="text" name="endTime2" style="width:40px;margin-top: 10px;float:left;" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
	              ...
	              <div  class="conTemp3">
	                 <!-- yyyyMMXU当前月（两种情况）-->
	                   ...
	                   <select id="houChose" name="houChose" style="float:left;">
	                        <option value="DIV1">候/2候…／6候表示“01/06/…/26”</option>
	                        <option value="SEQ1">1候/2候…／6候表示“01/02/…/06”</option>
		               </select>
		               <br/>
	                   <label class="control-label">开始时间：</label>
	                   <div class="currentMonth">当月-</div>
	              ...
	              <div class="conTemp4">
		               ...
		               <input type="text" name="startTime4" style="width:40px;margin-top: 10px;float:left;" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
		               <label class="control-label">月日：</label>              
		               <input type="text" name="startTime44" style="width:40px;margin-top: 10px;float:left;"/>
		               <br/>
		               <span style="color:red;">*月日填写规则：如1月1日填写0101</span> 
		               <br/>
		               <label class="control-label">结束时间：</label>
		               <div class="currentMonth">当年-</div>
		               <input type="text" name="endTime4" style="width:40px;margin-top: 10px;float:left;" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
		               <label class="control-label">月日：</label>
		               <input type="text" name="endTime44" style="width:40px;margin-top: 10px;float:left;"/>
		                <br/>
	          ...
	          <div class="defaultValue2">
		            ...
		            <input type="text" name="defaultValue2" style="width:40px;margin-top: 10px;float:left;"/>
		       ...
		       <!-- 台站列表 -->
		           <div class="StationList">
		             ...
		             <div class="stationItems">
		                 ...
		                 <input type="text" id="StationList_defaultvalue" name="StationList_defaultvalue" style="width:100px;margin-top: 10px;float:left;"/>
		             </div>
		             <div class="stationItems">
	...
	<c:if test="${groupcode=='ElementSel'}">
	    <div class="ElementSel">
	         ...
	         <input type="radio" name="ElementSel" value="ALLSel" checked="checked"/>所有要素
	...
	<c:if test="${groupcode=='ElementFilter'}">
	    <div class="ElementFilter">
	         ...
	         <input type="radio" name="ElementFilter" value="ALLSel" checked="checked"/>不过滤
	         <input type="radio" name="ElementFilter" value="EleFilter"/>要素过滤
	    </div>
	</c:if>
	<c:if test="${groupcode=='FormatSel'}">
	    <div class="formatTitle">名称：</div>
	    <input type="text" name="FormatSel_chname" id="FormatSel_chname" style="width:100px;margin-top: 10px;float:left;"/>
	</c:if>
	<c:if test="${groupcode=='QCSel'}">
	     <div class="formatTitle">名称：</div>
	    <input type="text" name="QCSel_chname" id="QCSel_chname" style="width:100px;margin-top: 10px;float:left;"/>
	</c:if>
	...
	&&groupcode!='StationSel'&&groupcode!='TimeSel'}">
	 <div class="ComConfigStyle">
	         ...
	         <div class="controls" style="margin-left:10px;margin-top:10px;float:left;">	
		         <input type="radio" name="ComConfig" value="ALLSel" checked="checked"/>权限
