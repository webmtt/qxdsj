...
$(function(){
	...
	$('.tlists').datagrid({    
		idField:'protype',
	    url:'statservs_statServs.action', 
	    title:'服务效益统计',
	    ...
	    columns:[[
	              ...
	              {field:'protype',title:'项目类型',width:250,sortable:true},
	              {field:'sum',title:'总量',width:150,sortable:true},
	              {field:'protype1',title:'项目类型',width:250,sortable:true},
	              {field:'sum1',title:'总量',width:150,sortable:true}
	    ]]    
	});  

	//初始化数据表格
	$('.tlists_detail').datagrid({
		idField:'project',
	    url:'statservs_statServsDetail.action', 
	    title:'重要项目清单',
	    loadMsg:'正在努力加载数据，请稍后...', //在从远程站点加载数据的时候显示提示消息。
	    singleSelect:false,
	    striped:true,
	    ...
	    columns:[[ 
	              {field:'ck',checkbox:true},
	              {field:'protype',title:'项目类型',width:160},
	              {field:'project',title:'项目信息',width:280},
//	              {field:'organ',title:'机构团体',width:250,sortable:true},
	...
	$("#expor").click(function(){
		var $tlists = $('.tlists');
		var rows  = $('.tlists').datagrid("getSelections");
		var jsonStr ;
		if(rows.length==0){
			var data   = $('.tlists').datagrid("getData");  
			jsonStr = JSON.stringify(data.rows)
		}else{
			jsonStr = JSON.stringify(rows);
		}
		window.open ('statservs_exporExcel.action?jsonStr='+jsonStr) ;
		
	});
	
	
	$("#expor2").click(function(){
		var $tlists_detail = $('.tlists_detail');
		...
		var url = "statservs_exporExcel2.action";
