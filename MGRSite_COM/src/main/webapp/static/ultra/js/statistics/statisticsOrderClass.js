...
$(function(){
	...
	$("#orgtype").combobox({
		...
		onSelect:function(rec){  
			 _zhbid.combobox({
                disabled: false,
                url: ctx + '/statistics/statisticsOrderTotal/statDepart?type='+rec.typeValue,
                valueField: 'typeValue',
                textField: 'typeName'
            }).combobox('clear');
	    },  
	 ...
	 $('#orgin').combobox({
			url : ctx+'/statistics/statisticsOrderTotal/statDepart?type=1',
			valueField : 'typeValue', // 值
			textField : 'typeName', // 显示的名称
			editable : false,
			width : 120,
			value : '0'
		});
...
function getData (start_time,end_time,timeType,org){
	$.ajax({
		url:ctx+'/statistics/statisticsOrderTotal/statClassByTime_picture2',
		type:'post',
		cache:false,
		dataType:'json',
		async:false,
	...
	$.ajax({
		url:ctx+'/statistics/statisticsOrderTotal/statClassByTime_list2',
		type:'post',
		cache:false,
		dataType:'json',
		async:false,
	...
	$('.tlists').datagrid({    
	   ...
	   fitColumns:true,
	    columns:[[    
	        ...
	        {field:'orderNumber',title:'订单量',width:110} 
	    ]]    
	});  
}
