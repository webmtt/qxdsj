	...
	$(function(){
		...
		$("#btn3").on("click",function(){
			...
			chart.xAxis[0].setCategories(chartData.center);
			chart.xAxis[0].setOptions({
					tickInterval:Math.round(chartData.center.length/6)
			});
			chart.series[0].remove(false);
			chart.addSeries({
				name:'总下载量',
...
function getData (start_time,end_time,timeType){
	$.ajax({
		url:ctx+'/statistics/statisticsDownloadTotal/statCenterPictureByTime2',
		type:'post',
		cache:false,
		dataType:'json',
		async:false,
...
function showtable(){
	...
	$('.tlists').datagrid({    
	    ...
	    url:ctx+'/statistics/statisticsDownloadTotal/statCenterListByTime2',  
	    data:{
			"start_time":start_time =="" ? "" : start_time,
			"end_time":end_time =="" ? "" : end_time,
			"timeType":timeType=="" ? "" : timeType
