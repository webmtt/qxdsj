...
public class BaseDao_ftp<T> {
  ...
  public int deleteById(Serializable id, String likeParentIds) {
    return update(
        "update "
            ...
            + " set delFlag = '"
            + BaseEntity.DEL_FLAG_DELETE
            + "' where id = :p1 or parentIds like :p2",
  ...
  public int updateDelFlag(Serializable id, String delFlag) {
    return update(
        "update " + entityClass.getSimpleName() + " set delFlag = :p2 where id = :p1",
  ...
  @SuppressWarnings("unchecked")
  public Page<T> find(
      Page<T> page, DetachedCriteria detachedCriteria, ResultTransformer resultTransformer) {
    // get count
    if (!page.isDisabled() && !page.isNotCount()) {
