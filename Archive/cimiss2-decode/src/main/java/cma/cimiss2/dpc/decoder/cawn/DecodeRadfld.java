 ...
 * @version 0.0.1
 */
public class DecodeRadfld {
	public static double defautValue = 999999;
	public static String defaultStr = "999999";
	SimpleDateFormat sFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	RadfldParseConfig cfg = RadfldParseConfig.getParaeConfig();
	/**
	 * 获取XML配置文件的内容
	 */
	List<RadfldParseBean> parseBeans = cfg.getParseBeans();
			...
			try {
				...
				if (lineTxt != null && lineTxt.size() >= 1) {
					for (int i = 0; i < lineTxt.size(); i++) {
						...
						if(RadfldParseConfig.IsCheck) {
							if(items.length >= RadfldParseConfig.reportLenth) {
								for (RadfldParseBean parseBean : parseBeans) {
...
//						sqls.add(sql.toString());
	...
	private void parseElementValue(List<Object> values, RadfldParseBean parseBean, String[] items, String fileName) {
	...
	private void calcElementValue(List<Object> values, RadfldParseBean parseBean, String[] items, String fileName) {
...
//				System.out.println(parseBean.getElementName().toUpperCase());
				if(expression.startsWith("[") && expression.endsWith("]")) {
					...
					try{
						...
						if(Integer.parseInt(item[1])==1) {
							value=Integer.parseInt(dt.substring(4, 6));
						}else if(Integer.parseInt(item[1])==2){
							value=Integer.parseInt(dt.substring(6, 8));
						}
						values.add(value);
					}catch (Exception e) {
						values.add(0);
				...
				}else if(expression.startsWith("D_DATETIME")) {
					try{
						int year =Integer.parseInt(items[2]);
						int JulianDay=Integer.parseInt(items[3]);
						String dt = DateUtil.convertJulianDay2Date(year, JulianDay); // 格式为：yyyyMMdd
	...
	private void parseDefaultElementValue(List<Object> values, RadfldParseBean parseBean, Date last_modifi_time) {
	...
	public static void main(String[] args) {
		...
		DecodeRadfld decodeHKFlash = new DecodeRadfld();
//		List<String> sqlList = processFile.decodeFile("D:\\observ_Japan_20180801080000_20180801083827.txt", ",", new Date());
		ParseResult<String> rs = decodeHKFlash.decodeFile("C:\\BaiduNetdiskDownload\\test\\G.0002.0011.R001\\G.0002.0011.R001\\20200315\\Z_CAWN_I_54084_20200315000000_O_RAD-FLD-MUL.TXT", ",", new Date());
