 ...
 */
public class SurfStatParseConfig {
	
	/** The parae config. */
	private static SurfStatParseConfig paraeConfig = null;
	
	/** The parse beans. */
	private List<SurfStatParseBean> parseBeans = null;
	 ...
	 */
	public static SurfStatParseConfig getParaeConfig() {
		if(paraeConfig == null) {
			paraeConfig =  new SurfStatParseConfig(config);
		}
		return paraeConfig;
	}
	 ...
	 */
	public List<SurfStatParseBean> getParseBeans() {
		return parseBeans;
	}
	 ...
	 */
	private SurfStatParseConfig(String config) {
		parseBeans = new ArrayList<SurfStatParseBean>();
		...
		if(configFile.exists() && configFile.isFile()) {
			...
			try {
		        ...
		        for (Element element : childList) {
		        	SurfStatParseBean bean = new SurfStatParseBean();
		        	// 获取要素名称
...
//						bean.setIndex(Integer.parseInt(element.attribute("index").getStringValue().trim()));
	...
	public static void main(String[] args) {
		SurfStatParseConfig.config = "config/CAWN_MET80_TAB.xml";
		SurfStatParseConfig.getParaeConfig();
		System.out.println(SurfStatParseConfig.reportLenth +"=========="+SurfStatParseConfig.IsCheck);
	}
