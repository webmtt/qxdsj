 ...
 */
public class RadfldParseConfig {
	
	/** The parae config. */
	private static RadfldParseConfig paraeConfig = null;
	
	/** The parse beans. */
	private List<RadfldParseBean> parseBeans = null;
	 ...
	 */
	public static RadfldParseConfig getParaeConfig() {
		if(paraeConfig == null) {
			paraeConfig =  new RadfldParseConfig(config);
		}
		return paraeConfig;
	}
	 ...
	 */
	public List<RadfldParseBean> getParseBeans() {
		return parseBeans;
	}
	 ...
	 */
	private RadfldParseConfig(String config) {
		parseBeans = new ArrayList<RadfldParseBean>();
		...
		if(configFile.exists() && configFile.isFile()) {
			...
			try {
		        ...
		        for (Element element : childList) {
		        	RadfldParseBean bean = new RadfldParseBean();
		        	// 获取要素名称
...
//						bean.setIndex(Integer.parseInt(element.attribute("index").getStringValue().trim()));
	...
	public static void main(String[] args) {
		RadfldParseConfig.config = "config/Light_3333_TAB.xml";
		RadfldParseConfig.getParaeConfig();
		System.out.println(RadfldParseConfig.reportLenth);
	}
