 ...
 */
public class Ocen158cwParseConfig {
	
	/** The parae config. */
	private static Ocen158cwParseConfig paraeConfig = null;
	
	/** The parse beans. */
	private List<Ocen158cwParseBean> parseBeans = null;
	 ...
	 */
	public static Ocen158cwParseConfig getParaeConfig() {
		if(paraeConfig == null) {
			paraeConfig =  new Ocen158cwParseConfig(config);
		}
		return paraeConfig;
	}
	 ...
	 */
	public List<Ocen158cwParseBean> getParseBeans() {
		return parseBeans;
	}
	 ...
	 */
	private Ocen158cwParseConfig(String config) {
		parseBeans = new ArrayList<Ocen158cwParseBean>();
		...
		if(configFile.exists() && configFile.isFile()) {
			...
			try {
		        ...
		        for (Element element : childList) {
		        	Ocen158cwParseBean bean = new Ocen158cwParseBean();
		        	// 获取要素名称
...
//						bean.setIndex(Integer.parseInt(element.attribute("index").getStringValue().trim()));
	...
	public static void main(String[] args) {
		Ocen158cwParseConfig.config = "config/CAWN_MET80_TAB.xml";
		Ocen158cwParseConfig.getParaeConfig();
		System.out.println(Ocen158cwParseConfig.reportLenth +"=========="+Ocen158cwParseConfig.IsCheck);
	}
