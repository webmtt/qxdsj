 ...
 */
public class OcenParseConfig {
	
	/** The parae config. */
	private static OcenParseConfig paraeConfig = null;
	
	/** The parse beans. */
	private List<OcenParseBean> parseBeans = null;
	 ...
	 */
	public static OcenParseConfig getParaeConfig() {
		if(paraeConfig == null) {
			paraeConfig =  new OcenParseConfig(config);
		}
		return paraeConfig;
	}
	 ...
	 */
	public List<OcenParseBean> getParseBeans() {
		return parseBeans;
	}
	 ...
	 */
	private OcenParseConfig(String config) {
		parseBeans = new ArrayList<OcenParseBean>();
		...
		if(configFile.exists() && configFile.isFile()) {
			...
			try {
		        ...
		        for (Element element : childList) {
		        	OcenParseBean bean = new OcenParseBean();
		        	// 获取要素名称
...
//						bean.setIndex(Integer.parseInt(element.attribute("index").getStringValue().trim()));
	...
	public static void main(String[] args) {
		OcenParseConfig.config = "config/CAWN_MET80_TAB.xml";
		OcenParseConfig.getParaeConfig();
		System.out.println(OcenParseConfig.reportLenth +"=========="+OcenParseConfig.IsCheck);
	}
