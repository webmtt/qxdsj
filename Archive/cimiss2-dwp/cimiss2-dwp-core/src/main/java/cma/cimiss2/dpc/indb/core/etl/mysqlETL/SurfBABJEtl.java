...
import cma.cimiss2.dpc.indb.core.tools.LoadPropertiesFile;
...
public class SurfBABJEtl extends AbstractEtl<SurfaceObservationDataNation> {
...
//                    date.setMinutes(j);
                 ...
                 // 2020-3-31 chy date.getMinutes() 改为 date.getMinutes() - 1
                    if(date.getMinutes() >= 56){
                    	...
                    	dataMinPre.put("V13011", bean.getPrecipitationEveryMinutes().get(59).getValue());
                    	dataMinPre.put("Q13011", bean.getPrecipitationEveryMinutes().get(59).getQuality().get(1).getCode());
                   
                    }
//                    dataMinPre.put("V13011", bean.getPrecipitationEveryMinutes().get(date.getMinutes()).getValue());
//                    dataMinPre.put("Q13011", bean.getPrecipitationEveryMinutes().get(date.getMinutes()).getQuality().get(1).getCode());
                    statTF.getMinPreList().add(dataMinPre);
                }
            }
