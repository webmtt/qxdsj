...
import cma.cimiss2.dpc.decoder.bean.detection.NafpChnNbrcntPreTab;
import cma.cimiss2.dpc.decoder.detection.DecodeNbrCnt;
...
public class NbrCntEtl extends AbstractEtlNew<NafpChnNbrcntPreTab> {
    ...
    @Override
    public ParseResult<NafpChnNbrcntPreTab> extract(File file) {
    	DecodeNbrCnt decodeSS = new DecodeNbrCnt();
        String path = file.getPath();
        ParseResult<NafpChnNbrcntPreTab> decode = (ParseResult<NafpChnNbrcntPreTab>) decodeSS.decode(file, new HashSet<String>());
        return decode;
    }
    /**
     * 报文格式化
     ...
     */
    @Override
    public StatTF transform(ParseResult<NafpChnNbrcntPreTab> result, File file, String... tables) {
//        reload(intervalTime);
        //返回集合
        Map<String, Object> allMap = new HashMap<>();
        //解码数据集合
        List<NafpChnNbrcntPreTab> beans = result.getData();
        //报文集合
...
//        String repTable = tables[2];
        ...
        String D_DATA_ID = "F.0052.0005.S001";

        //数据处理
        for (int i = 0; i < beans.size(); i++) {
            NafpChnNbrcntPreTab bean = beans.get(i);
            ...
            String D_RECORD_ID = time + "000000_" + ssss+"_" +bean.getFcstTime()+"_"+bean.getFcstVar()+"_"+bean.getObsVar()+"_"+bean.getVxMask()+"_"+ bean.getPnts()+"_"+bean.getFbs();
...
//			data.put("V_MODEL", bean.getModel());
			...
			data.put("V_FBS", bean.getFbs());
			data.put("V_FSS", bean.getFss());
			data.put("V_AFSS", bean.getAfss());
