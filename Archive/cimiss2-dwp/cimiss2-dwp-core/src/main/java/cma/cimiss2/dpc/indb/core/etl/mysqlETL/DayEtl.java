...
import cma.cimiss2.dpc.decoder.bean.detection.RadiMulChnDayTab;
import cma.cimiss2.dpc.decoder.detection.DecodeD;
import cma.cimiss2.dpc.indb.core.bean.StatTF;


/**
 * 降水MPR检验评估结果类 mysql缓冲库
 */
@IocBean(fields = {"rdb", "cimiss"},singleton = false)
public class DayEtl extends AbstractEtlNew<RadiMulChnDayTab> {
    ...
    @Override
    public ParseResult<RadiMulChnDayTab> extract(File file) {
        DecodeD decodeSS = new DecodeD();
        String path = file.getPath();
        ParseResult<RadiMulChnDayTab> decode = (ParseResult<RadiMulChnDayTab>) decodeSS.decode(file, new HashSet<String>());
        return decode;
    }
    /**
     * 报文格式化
     ...
     */
    @Override
    public StatTF transform(ParseResult<RadiMulChnDayTab> result, File file, String... tables) {
//        reload(intervalTime);
        //返回集合
        Map<String, Object> allMap = new HashMap<>();
        //解码数据集合
        List<RadiMulChnDayTab> beans = result.getData();
        //报文集合
...
//        String repTable = tables[2];
        ...
        String D_DATA_ID = "F.0052.0001.S001";

        //数据处理
        for (int i = 0; i < beans.size(); i++) {
        	RadiMulChnDayTab bean = beans.get(i);
...
//			data.put("D_DATETIME",bean.getoEndTime()); //资料时间
