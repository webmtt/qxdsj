...
public class DayLightEtl extends AbstractEtl<SurfWeaChnSsdHor> {
    ...
    public ParseResult<SurfWeaChnSsdHor> extract(File file) {
        ...
        ParseResult<SurfWeaChnSsdHor> decode;
        if (QC != null && QC.equals("0")) {
        	decode = (ParseResult<SurfWeaChnSsdHor>) decodeSS.decodeNoQC(file, new HashSet<String>());
        } else {
        	decode = (ParseResult<SurfWeaChnSsdHor>) decodeSS.decode(file, new HashSet<String>());
        }
        return decode;
    }
    /**
     ...
     */
    @Override
    public StatTF transform(ParseResult<SurfWeaChnSsdHor> result, File file, Map<String,String> codeMap, String... tables) {
//        reload(intervalTime);
        //返回集合
        ...
        for (int i = 0; i < beans.size(); i++) {
            ...
            dataDay.put(".table", dataDayTable);
            dataDay.put("D_RECORD_ID", D_RECORD_ID);//记录标识
            dataDay.put("D_DATA_ID", day_sod_code);//资料标识
            ...
            dataDay.put("D_SOURCE_ID", getEvent());//CTS编码
...
//            dataDay.put("V04003", observationTime.getDate());
            ...
            data.put("D_DATA_ID", ssd_sod_code);//资料标识
