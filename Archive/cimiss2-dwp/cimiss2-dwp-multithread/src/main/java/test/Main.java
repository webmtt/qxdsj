...
{ 
	...
	{		
		...
		//String filename = "测试数据_测试数据/F.0011.0002.R001/Z_NWGD_C_BEPK_20190520155738_P_RFFC_SPCC-EDA10_201905202000_24003.GRB2"; //北京区域模式，与日本高分格式一样
		
		//String filename = "D:/大数据平台-存储设计-2017/与短临平台-SWAN-对接/北上广区域模式/Z_NAFP_C_BEPK_20190919120000_P_3km-RMAPS-f09.BIN"; //华北区域模式
		
		//String filename = "D:/大数据平台-存储设计-2017/数据模式解码-大数据平台/grib_decode/测试数据_测试数据/CMPA-A.0042.0005.S001/Z_SURF_C_BABJ_20191113145234_P_CMPA_FRT_CHN_0P05_3HOR-PRE-2019111314.GRB2"; //CMPA降水：
		
		//String filename = "D:/F.F/Z_NWGD_C_BABJ_20200428112021_P_RFFC_SCMOC-RAT_202004280500_03001.GRB2.BEKM";
		//String filename = "D:/F.F/Z_NWGD_C_BABJ_20200428112021_P_RFFC_SCMOC-RAT_202004280500_03001.GRB2";
		
		//String filename = "D:/大数据平台-存储设计-2017/数据模式解码-大数据平台/grib_decode/测试数据_测试数据/C.0011.0001.R001/Z_OCEN_C_BABJ_20200425013221_P_CODAS_GLB_0P25_DAY-SST-20200424.nc";
		
		//String filename = "D:/大数据平台-存储设计-2017/数据模式解码-大数据平台/grib_decode/测试数据_测试数据/C.0011.0001.R001/Z_OCEN_C_BABJ_20200425013221_P_CODAS_GLB_0P25_DAY-SST-20200424.nc";
		
		String filename = "D:/大数据平台-存储设计-2017/数据模式解码-大数据平台/grib_decode/测试数据_测试数据/F.0010.0002.R001/W_NAFP_C_ECMF_20200503054247_P_C1D05030000050300001.bz2";
		String d_data_id = "F.0010.0002.R001"; //四级编码
		
		GribConfig.readConfig();
		
		GribConfigAreaMapping.readConfig();
		
		GribM4Config.readConfig();
		...
		*/
		...
		//CassandraM4DbOperation cassadraOperM4 = new CassandraM4DbOperation();
		CassandraM4DbOperation cassadraOperM4 = null;
		GribDecode grib_decode = new GribDecode();
		...
		if(index_type==0) //GRIB拆分入库
		{
			...
			{					
				...
				ret_str = grib_decode.split_grib_record_use_S0Lengh(filename, d_data_id, rcv_time,cassadraOper,cassadraOperM4,true,false);	
			}
			...
			{				
				...
				//返回：解码成功场数_入Cassandra场数_入索引表场数_文件读取错误
				ret_str = grib_decode.split_grib_record_other(filename, d_data_id, rcv_time,cassadraOper,cassadraOperM4,true,false);
			}
		}
		else if(index_type==1) //解析文件名+入Cassandra(智能网格GRIB、nc产品)
		{
			//解析文件名，并入索引表
			String ret_nwfd = grib_decode.process_nwfd_file(filename, d_data_id, rcv_time);
			...
			if(grib_version.equalsIgnoreCase("1")) //grib版本1
			{
				...
				map_grib_decode_result1.clear();	
				map_grib_decode_result1 = grib_decode.read_grib1_data(filename, d_data_id, rcv_time);
				System.out.println("map_grib_decode_result1.size:" +map_grib_decode_result1.size());					
				
				int sucess1 = cassadraOper.into_cassandra_map(map_grib_decode_result1);
				
				System.out.println("sucess1=" + sucess1);
			...
			else if(grib_version.equalsIgnoreCase("2")) //grib版本2
			{
				...
				map_grib_decode_result2.clear();	
				map_grib_decode_result2 = grib_decode.read_grib2_data(filename, d_data_id, rcv_time);
				System.out.println("map_grib_decode_result2.size:" +map_grib_decode_result2.size());				
				
					
				int sucess2 = cassadraOper.into_cassandra_map(map_grib_decode_result2);			
				
				System.out.println("sucess2=" + sucess2);
			...
			{
				System.out.println("field_organize:" + field_organize);
			}
		}
		else if(index_type==2) //只解析文件名(jpg文件等)
		{
			//只解析文件名
			String ret_nwfd = grib_decode.process_nwfd_file(filename, d_data_id, rcv_time);
		}
