...
public class RabbitMQConfig {
	...
	public RabbitMQConfig(String path) {
		...
		setQueueName(pps.getProperty("RabbitMQ.queueName").trim());
		setHost(pps.getProperty("RabbitMQ.host").trim());
		setUser(pps.getProperty("RabbitMQ.user").trim());
		setPassword(pps.getProperty("RabbitMQ.passWord").trim());
		setPort(Integer.parseInt(pps.getProperty("RabbitMQ.port", "5672").trim()));
	}
