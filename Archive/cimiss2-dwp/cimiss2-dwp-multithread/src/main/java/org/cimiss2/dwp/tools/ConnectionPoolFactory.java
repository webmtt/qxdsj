...
import cma.cimiss2.dpc.indb.common.Base64Util;
/**
 * -------------------------------------------------------------------------------
 * <br>
...
public class ConnectionPoolFactory {
	...
	private  DruidDataSource dataSourceFILEINDEX = null;
	private  DruidDataSource dataSourceXUGU = null;
	 ...
	 * @throws SQLException : DruidPooledConnection    数据库连接对象
	 */
		...
		try {
			if(dataSourceCIMISS == null && properties != null) {
				synchronized (this) {
					...
					dataSourceCIMISS.setBreakAfterAcquireFailure(false);
					dataSourceCIMISS.setConnectionErrorRetryAttempts(10);
					dataSourceCIMISS.setTimeBetweenConnectErrorMillis(1000*5);
					// 
		...
		try {
			if(dataSourceRDB == null && properties != null) {
				synchronized (this) {
					...
					// 归还连接时执行validationQuery检测连接是否有效
					dataSourceRDB.setBreakAfterAcquireFailure(false);
					dataSourceRDB.setConnectionErrorRetryAttempts(10);
					dataSourceRDB.setTimeBetweenConnectErrorMillis(1000*5);
		...
		try {
			if(dataSourceFILEINDEX == null && properties != null) {
				synchronized (this) {
					...
					dataSourceFILEINDEX.setBreakAfterAcquireFailure(false);
					dataSourceFILEINDEX.setConnectionErrorRetryAttempts(10);
					dataSourceFILEINDEX.setTimeBetweenConnectErrorMillis(1000*5);
...
//					dataSourceXUGU.setFilters(properties.getProperty("xugu.filters").trim()); //xugu加此行报错
					...
					dataSourceXUGU.setBreakAfterAcquireFailure(false);
					dataSourceXUGU.setConnectionErrorRetryAttempts(10);
					dataSourceXUGU.setTimeBetweenConnectErrorMillis(1000*5);
					dataSourceXUGU.setNotFullTimeoutRetryCount(10);
	...
	public static void main(String[] args) {
		System.out.println(ConnectionPoolFactory.getInstance().getConnection("cimiss"));
	}
