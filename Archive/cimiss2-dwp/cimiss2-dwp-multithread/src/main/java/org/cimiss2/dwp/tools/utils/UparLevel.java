...
//		System.out.println(uparLevel.getConvectionLvs());
		int a = 131072 + 8196 + 4096 + 2048 + 8 + 4 + 2;
		List<Integer> list = calLevels(uparLevel, String.valueOf(a), 18);
	...
	public static List<Integer> calLevels(UparLevel levels, String detectMeans, int bitWidth){
		...
		try{
			...
			for(int i = 0; i < length; i ++){
				if(aString.charAt(i) == '1'){
					verticals.add((int)Math.pow(2, bitWidth - i - 1));
					if(i == 1) //  
						levels.setStandardLvs(levels.getStandardLvs() + 1);
					else if(i == 2) // 
						levels.setConvectionLvs(levels.getConvectionLvs() + 1);
