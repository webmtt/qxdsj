...
/**
 * @Description: 雨量站文件轮询主类
 * @Aouthor: xzh
 * @create: 2018-05-21 11:46
 */
public class dpc_surf_perf {
    ...
    public void run() {
		...
		for (int i = 0; i < count; i++) {
			switch (dataSource) {
			case "2":
				executor.execute(new PERFMysqlSubThread());
				break;
			case "1":
				executor.execute(new PERFMysqlSubThread());
				break;
        ...
        executor.execute(new FilePolling(src,"A.0001.0029.R001:"));
        for (int i = 0; i < count; i++) {
            executor.execute(new FilePERFSubThread());
        }
        executor.shutdown();*/
    }
