 ...
 * 
 * <br>
 * @Title:  TurbidityMainThread  
 * @Description:    TODO(大气浑浊度观测资料数据)
 * 大气成分	沙尘暴大气浑浊度（NEP）	大数据平台	G.0001.0004.R001
 * <pre>
 * SOFTWARE HISTORY
 * Date         Engineer    Description
 * ------------ ----------- --------------------------
 * 2018年1月25日 上午10:48:07   dengyongliang    Initial creation.
 * </pre>
 * 
 ...
 *
 */
public class TurbidityMainThread {;
    ...
    if(args.length < 1) {
 		...
 		return;
			...
			if(args.length <2) {
 			...
 			//File file = new File("config/cawn_sand_nep_config.properties");
	        ...
	        if(StartConfig.fileLoop() == 0) {
		        for (int i = 0; i < threadCount; i++) {
		            executor.execute(new TurbiditySubThread(diQueues));
		        }
	        }
