 ...
 * @Description:    TODO(地面定时天气数据（ISM/ISN/ISA/ISI）（BUFR格式）	大数据平台	A.0001.0018.R001)
 * @author: zhangliang
 * @date:   2019年9月19日 13:42
...
//		args=new String[] {"5088","config/dpc_bufr_global_ocean_argo.properties"};//轮训测试用
		if(args.length < 1) {
			...
			if(args.length <2) {
 			...
 			File file = new File(args[1].trim()); //config/dpc_bufr_global_ocean_argo.properties
