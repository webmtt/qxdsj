 ...
 * 
 * <br>
 * @Title:  NSDMainThread.java
 * @Package cma.cimiss2.dpc.indb.cawn.nsd
 * @Description:(大气成分气溶胶数浓度（NSD）资料入库主线程)
 * 大气成分	大气成分气溶胶数浓度（NSD）	大数据平台	G.0002.0004.R001
 * <pre>
 * SOFTWARE HISTORY
 * Date         Engineer    Description
 * ------------ ----------- --------------------------
 * 2018年3月30日 上午10:40:07   maxiyue   Initial creation.
 * </pre>
 * 
 * @author maxiyue
 */

public class NSDMainThread {
    ...
    public static void main(String[] args) throws SQLException, InterruptedException {
        if(args.length < 1) {
 			...
 			//File file = new File("config/cawn_snsd_config.properties");
	        ...
	        // 消息中间件
	     	if(StartConfig.fileLoop() == 0) {
		        for (int i = 0; i < threadCount; i++) {
		            executor.execute(new NSDMultSubThread(diQueues));
		        }
	     	}
