 ...
 * -------------------------------------------------------------------------------
 * <br>
 * @Title:  AcidRainMainThread.java   
 * @Package cma.cimiss2.dpc.indb.cawn.acidrain   
 * @Description:    TODO(大气成分酸雨日数据数据处理主线程)
 ...
 * ------------ ----------- --------------------------
 * 2018年2月11日 上午11:35:57   maxiyue    Initial creation.
 * </pre>
 * 
 * @author maxiyue
 *---------------------------------------------------------------------------------
...
public class AcidRainMainThread {
        ...
        if(args.length < 1) {
			...
			if(StartConfig.fileLoop() == 0) {
		        for (int i = 0; i < threadCount; i++) {
		            executor.execute(new AcidRainMultSubThread(diQueues));
		        }
			}
