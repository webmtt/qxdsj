...
//        timer.schedule(new MyTask(dayValMainThread),0,600000);
    ...
    public void run() {
        ...
        if  ("1".equals(ConfigurationManager.getString("fileloop"))) {
            executor.execute(new FilePolling(ConfigurationManager.getString("src"), "SURF_F.0052.0004.S001:"));
        }
        for (int i = 0; i < count; i++) {
          
            executor.execute(new TenValueMysqlSubThread());
