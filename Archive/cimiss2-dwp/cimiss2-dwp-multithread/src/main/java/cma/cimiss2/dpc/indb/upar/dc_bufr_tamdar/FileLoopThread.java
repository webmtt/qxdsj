...
import cma.cimiss2.dpc.indb.upar.dc_bufr_tamdar.service.OTSService;
import cma.cimiss2.dpc.indb.upar.dc_bufr_tamdar.service.UparTamdarServiceImpl;
...
public class FileLoopThread implements Runnable{
	...
	public FileLoopThread(String tableSection,BlockingQueue<String> files, BlockingQueue<StatDi> diQueues) {
		...
		this.files = files;
		UparTamdarServiceImpl.setDiQueues(diQueues);
				...
				if(file.exists() && file.isFile()) {
					...
					try{
						if(StartConfig.getDatabaseType() == 0 || StartConfig.getDatabaseType() == 1){
							...
							decode = uparArdServiceImpl.decode(filepath, tableSection, tableList);
						}
						else {
