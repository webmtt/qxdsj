 ...
 */
public class FilePERFSubThread implements Runnable {
    ...
    private cma.cimiss2.dpc.indb.core.etl.otsETL.SurfPERFEtl surfPERFEtlOTS;
    ...
    public FilePERFSubThread() {
        ...
        if (dataSource.equals("2")) {
            surfPERFEtlOTS = new cma.cimiss2.dpc.indb.core.etl.otsETL.SurfPERFEtl();
            surfPERFEtlOTS.setPath(configPath);
            ...
            surfPERFEtlMysql.setDataSource(dataSource);
...
//            System.out.println("message==="+message);
            ...
            if (dataSource.equals("2")) {
                surfPERFEtlOTS.setEvent(event);
                surfPERFEtlOTS.setFile(file);
            } else {
                surfPERFEtlMysql.setEvent(event);
                surfPERFEtlMysql.setFile(file);
            }
            buffer.append("INFO : " + new Date() + " begin : " + message + "\n");
            //解码
            ParseResult<PrecipitationObservationDataReg> result = null;
            try {
                if (dataSource.equals("2")) {
                    result = surfPERFEtlOTS.extract(file);
                } else {
                    result = surfPERFEtlMysql.extract(file);
                }
                errorList = result.getError();
                if (result.isSuccess()) {//全部解码成功
                    //格式化数据
                    StatTF statPERF = null;
                    if (dataSource.equals("2")) {
                        statPERF = surfPERFEtlOTS.transform(result, file, horTableHelper, minTableHelper, repTableHelper);
                    } else {
                        statPERF = surfPERFEtlMysql.transform(result, file, codeMap, horTable, minTable, minPreTable, cumPreTable, repTable);
                    }
                    ...
                    try {
                        if (dataSource.equals("2")) {
                            loadHor = surfPERFEtlOTS.load(horList, horTableHelper, false);
                        } else {
                            //小时表
                    ...
                    try {
                        if (dataSource.equals("2")) {
                            loadMin = surfPERFEtlOTS.load(minList, minTableHelper, false);
                        } else {
                    ...
                    try {
                        if (dataSource.equals("2")) {
                            loadRep = surfPERFEtlOTS.load(reports, repTableHelper, false);
                        } else {
