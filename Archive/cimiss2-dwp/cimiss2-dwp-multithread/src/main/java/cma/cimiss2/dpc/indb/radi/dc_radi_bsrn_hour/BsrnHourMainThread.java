...
import cma.cimiss2.dpc.indb.radi.dc_radi_bsrn_hour.FileLoopThread;
 ...
 * 
 * <br>
 * @Title:  BsrnHourMainThread.java
 * @Package org.cimiss2.dwp.z_radi.bsrn.hour
 * @Description:(正点基准辐射数据文件				D.0011.0002.R001
 * <pre>
 * SOFTWARE HISTORY
 * Date         Engineer    Description
 * ------------ ----------- --------------------------
 * 2018年3月26日 下午2:55:59   maxiyue   Initial creation.
 * </pre>
 * 
 * @author maxiyue
 */
public class BsrnHourMainThread {
 ...
 // 声明一个缓存队列，存放轮询到的文件路径
    ...
    public static void main(String[] args) throws SQLException, InterruptedException {
        if(args.length < 1) {
 			...
 			//File file = new File("config/dpc_radi_bsrn_hour.properties");
	        ...
	        // 消息中间件
	     	if(StartConfig.fileLoop() == 0) {
		        for (int i = 0; i < threadCount; i++) {
		            executor.execute(new BsrnHourMultSubThread(diQueues));
		        }
	     	}
