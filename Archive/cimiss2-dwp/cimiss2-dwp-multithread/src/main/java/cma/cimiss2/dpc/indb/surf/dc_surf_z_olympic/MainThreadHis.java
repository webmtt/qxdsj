...
import cma.cimiss2.dpc.indb.common.LoopFolderThreadHis;
/**
 * 
 * *******************************************************************************************<br>
...
//                    new ThreadPoolExecutor.DiscardPolicy());
            if (StartConfig.fileLoop() == 0)  {
                ...
                for (int i = 0; i < threadCount; i++) {
                	executor.execute(new SubThreadHis(diQueues, diQueues2));
                }
            } else if (StartConfig.fileLoop() == 1) {
                // 目录轮询
                for (int i = 0; i < threadCount; i++) {
//                	executor.execute(new FileLoopThread(files, diQueues, diQueues2));
                }
                ExecutorService service = Executors.newFixedThreadPool(1);
                service.execute(new LoopFolderThreadHis(files));
            }
            // DISubThread
//            ExecutorService service = Executors.newFixedThreadPool(1);
