...
import cma.cimiss2.dpc.indb.ocen.dc_ocean_tide.FileLoopThread;

/**
 * 
 * <br>
 * @Title:  TideMainThread.java   
 * @Package cma.cimiss2.dpc.indb.ocen.tide   
 * @Description:(全球潮位站观测资料入库主函数)  
 * 海洋局潮位资料	大数据平台	C.0001.0025.R001
 * <pre>
 * SOFTWARE HISTORY
 * Date         Engineer    Description
 * ------------ ----------- --------------------------
 * 2018年3月5日 上午10:54:45   cuihongyuan    Initial creation.
 * </pre>
 * 
 * @author cuihongyuan
 *
    ...
    public static void main(String[] args) {
    	if(args.length < 1) {
			...
			if(args.length <2) {
 			...
 			//File file = new File("config/dpc_ocen_tide_nodi.properties");
	        ...
	        // 消息中间件
	     	if(StartConfig.fileLoop() == 0) {
		        for (int i = 0; i < threadCount; i++) {
		        	executor.execute(new TideMultSubThread(diQueues));
		        }
	     	}
