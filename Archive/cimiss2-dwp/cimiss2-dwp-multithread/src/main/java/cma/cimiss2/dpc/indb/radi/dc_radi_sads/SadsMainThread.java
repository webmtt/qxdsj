...
import cma.cimiss2.dpc.indb.radi.dc_radi_sads.FileLoopThread;
/**
 * 
 * <br>
 * @Title:  SadsMainThread.java
 * @Package cma.cimiss2.dpc.indb.radi.sads
 * @Description:(国家站气象辐射小时观测资料	D.0001.0002.R001)
 * 
 * <pre>
 * SOFTWARE HISTORY
 * Date         Engineer    Description
 * ------------ ----------- --------------------------
 * 2018年3月22日 下午4:24:59   maxiyue   Initial creation.
 * </pre>
 * 
 * @author maxiyue
 */
public class SadsMainThread {
 ...
 // 声明一个缓存队列，存放轮询到的文件路径
    ...
    public static void main(String[] args) throws SQLException, InterruptedException {
        if(args.length < 1) {
 			...
 			//File file = new File("config/dpc_radi_sads.properties");
	     ...
	     // 消息中间件
	     	if(StartConfig.fileLoop() == 0) {
		        for (int i = 0; i < threadCount; i++) {
		            executor.execute(new SadsMultSubThread(diQueues));
		        }
	     	}
