 ...
 * 
 * <br>
 * @Title:  SandPmmMainThread.java
 * @Package cma.cimiss2.dpc.indb.sand.pmm
 * @Description:    (沙尘暴气溶胶质量浓度（PMM）)
 * 大气成分	沙尘暴气溶胶质量浓度（PMM）	大数据平台	G.0001.0007.R001

 * <pre>
 * SOFTWARE HISTORY
 * Date         Engineer    Description
 * ------------ ----------- --------------------------
 * 2018年3月16日 下午3:32:40   maxiyue   Initial creation.
 * </pre>
 * 
 * @author maxiyue
 */
public class SandPmmMainThread {
	 // 声明一个缓存队列
    ...
    */
    public static void main(String[] args) {
        if(args.length < 1) {
 			...
 			//File file = new File("config/cawn_sand_pmm_config.properties");
        ...
        // 启动固定线程池
	        ...
	        if(StartConfig.fileLoop() == 0) {
		    	for (int i = 0; i < threadCount; i++) {
		    		executor.execute(new SandPmmMultSubThread(diQueues));
		    	}
	        }
