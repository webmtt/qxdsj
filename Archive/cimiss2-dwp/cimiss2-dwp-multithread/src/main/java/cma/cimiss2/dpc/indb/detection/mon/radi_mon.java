package cma.cimiss2.dpc.indb.detection.mon;
...
public class radi_mon {
    ...
    public static void main(String[] args) throws SQLException, InterruptedException {
        ...
        LogUtil.info("loggerInfo", "降水检测的计数文件" + new Date());
//        LogUtil.info("loggerInfo", "格点降水要素NBRCNT检验评估结果文件" + new Date());
        ...
        radi_mon dayValMainThread = new radi_mon();
    ...
    public void run() {
        ...
        if  ("1".equals(ConfigurationManager.getString("fileloop"))) {
            executor.execute(new FilePolling(ConfigurationManager.getString("src"), "PRE_F.0052.0002.S001:"));
        }
        for (int i = 0; i < count; i++) {
           
            executor.execute(new MonValueMysqlSubThread());
...
/*class MyTask extends TimerTask{
	...
	private radi_mon dayValMainThread;
	
    public MyTask(radi_mon dayValMainThread) { 
