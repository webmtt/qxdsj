 ...
 * 
 * <br>
 * @Title:  ForecastLastMainThread.java   
 * @Package cma.cimiss2.dpc.indb.sevp.cityforecastlast 
 * @Description:    TODO(全国城镇预报指导预报产品       大数据平台	M.0012.0004.R001
 * <pre>
 * SOFTWARE HISTORY
 * Date         Engineer    Description
 * ------------ ----------- --------------------------
 * 2018年3月21日 下午7:02:23   cuihongyuan    Initial creation.
 * </pre>
 * 
 * @author cuihongyuan
 *
...
public class ForecastLastMainThread {
    // 声明一个缓存队列
    ...
    public static void main(String[] args) {
        if(args.length < 1) {
			...
			if(args.length <2) {
 			...
 			//File file = new File("config/sevp_city_config.properties");
	        ...
	        // 消息中间件
	     	if(StartConfig.fileLoop() == 0) {
		        for (int i = 0; i < threadCount; i++) {
		            executor.execute(new ForecastLastMultSubThread(diQueues));
		        }
	     	}
