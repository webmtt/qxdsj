 ...
 * 
 * <br>
 * @Title:  ASPMainThread.java
 * @Package cma.cimiss2.dpc.indb.cawn.asp
 * @Description:(散射特性（ASP）监测项目小时资料入库主线程)
 * 大气成分	大气成分气溶胶散射特性（ASP）	大数据平台	G.0002.0003.R001
 * <pre>
 * SOFTWARE HISTORY
 * Date         Engineer    Description
 * ------------ ----------- --------------------------
 * 2018年4月4日 下午3:12:34   maxiyue   Initial creation.
 * </pre>
 * 
 * @author maxiyue
 */

public class ASPMainThread {
    ...
    public static void main(String[] args) throws SQLException, InterruptedException {
        if(args.length < 1) {
 			...
 			//File file = new File("config/cawn_aasp_config.properties");
	        ...
	        // 消息中间件
	     	if(StartConfig.fileLoop() == 0) {
		        for (int i = 0; i < threadCount; i++) {
		            executor.execute(new ASPMultSubThread(diQueues));
		        }
	     	}
