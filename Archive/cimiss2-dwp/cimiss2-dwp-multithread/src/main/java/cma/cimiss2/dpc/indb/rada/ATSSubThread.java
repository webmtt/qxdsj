	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				List<FileInfo> fileInfos = ServiceQueue.getServerQueue().get(5);
...
//								}
								...
								if (tableInfo.getStation().getType() == 0) {
									...
									fileData.setProdCode(sp[tableInfo.getStation().getPos()].substring(
											tableInfo.getStation().getStart(), tableInfo.getStation().getEnd()));
								} else {
...
//									fileData.setProdCode(tableInfo.getStation().getStr());
								...
								if (tableInfo.getProdCode().getType() == 0) {
									fileData.setReserveKeyOne(tableInfo.getProdCode().getStr());
								} else if (tableInfo.getProdCode().getType() == 1) {
									fileData.setReserveKeyOne(sp[tableInfo.getProdCode().getPos()]);
								} else if (tableInfo.getProdCode().getType() == 2) {
									fileData.setReserveKeyOne(sp[tableInfo.getProdCode().getPos()].substring(
...
//									boolean bret=ats.createStaFileDataTable(tableConfig.getAtsTable(),30*24*60*60);
									...
									try {
										...
										System.out.println(fileData.getDataId());
										System.out.println(fileData.getDataName());
										System.out.println(fileData.getData().length);
										result = ats.writeCommonData(tableConfig.getAtsTable(), fileData);
										System.out.println("ats.writeCommonData........................"+ (System.currentTimeMillis() - ll));
									} catch (Exception e) {
										e.printStackTrace();
...
//										sendEi(EIEventType.CASSANDRA_CONNECT_ERROE);
								...
								System.out.println(
										"this.diQueues.offer ----------------" + i + "    and  " + fileInfos.size());
								this.diQueues.offer(fileDI);
							} else {
								logger.error("\n file not find ：" + filePath);
							}
						} else {
							logger.info(
