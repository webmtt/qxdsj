 ...
 */
public abstract class BaseThreadOTS<T> implements Runnable {
...
//    protected ThreadLocal<Integer> errorNum = new ThreadLocal<Integer>() {
        ...
        try {
            ...
            Consumer consumer = new DefaultConsumer(channel) {
                ...
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                    ...
                    HashMap<String, Integer> writeMap = null;
                    try {
                        StatFormate<T> formate = formate(message, buffer, diInfo, eiInfo);
                        ParseResult<T> result = formate.getResult();
                        File file = formate.getFile();
                        List<ReportError> errorList = result.getError();
                        if (result.isSuccess()) {
                             writeMap = write(message, result, file, buffer, diInfo, eiInfo);
...
//            LOGGER.error("EI配置文件中没有事件类型：" + event_type);
    ...
    public abstract HashMap<String, Integer> write(String message, ParseResult<T> result, File file, StringBuffer buffer, List<RestfulInfo> diInfo, List<RestfulInfo> eiInfo);

    public abstract StatFormate<T> formate(String message, StringBuffer buffer, List<RestfulInfo> diInfo, List<RestfulInfo> eiInfo);

    public abstract void finallyAccept(HashMap<String, Integer> map, StringBuffer buffer);
