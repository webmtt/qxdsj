package cma.cimiss2.dpc.indb.cawn.dc_cawn_mee.service;
							 ...
							 if (e1.getErrorCode() == 13001 || e1.getErrorCode() == 1062) {//主键冲突异常，则进行更新入库（先删除后插入）
								 ...
								 String currentsql=sqls.get(j);
									...
									if(va.length >= 4){
										...
										if(ColVal.containsKey("V_starttime"))
											startdt = ColVal.get("V_starttime");
										if(ColVal.containsKey("V_endtime"))
											enddt = ColVal.get("V_endtime");
									}
...
////				Pstmt = connection.prepareStatement(sql);
	...
	public static int deleteExistData2(String stationname,String datetime,String min,String second,String tableName,Connection connection){
		Statement Pstmt = null;
		String deletesql = "delete from "+tableName+" "
				+ "where  D_DATETIME = "+ "'" +datetime+ "'" + " and V01301 =" + "'" +stationname+ "'" +" and v04005= "+ "'" +min+ "'" +" and v04006= "+ "'" +second+ "'" ;
		int dsql=0;
		try{
			if(connection != null){	
////				Pstmt = connection.prepareStatement(sql);
