package cma.cimiss2.dpc.decoder.fileDecode.common.decode;

import cma.cimiss2.dpc.decoder.bean.radi.RadiDigChnMulTab;
import cma.cimiss2.dpc.decoder.fileDecode.common.bean.*;
import cma.cimiss2.dpc.decoder.fileDecode.util.reflect.ReflectUtil;
...
public class Decoding {
    ...
    public MeadowValueTab meadowRadiHandle(String meadowCts,List<RadiDigChnMulTab> radiValueList,FirstData sf, String type) {
        ...
        String dataTime=meadowValueTab.getV04001()+"-"+meadowValueTab.getV04002();
    ...
    public DayValueTab dayVlaueHandle(DayValueTab dayValueTab,String dayCtsCode,TimeEntity timeEntity, ViewParamDayEntity dayEntity,FirstData sf,HourValueTab hourValueTab,int size) {
        ...
        //资料时间
        String time=dayValueTab.getV04001()+"-"+dayValueTab.getV04002()+"-"+dayValueTab.getV04003();
        ...
        // 日平均海平面气压 ( 单位：百帕 )
        averageHandle(dayValueTab,"V10051_701","V10051",hourValueTab,size);
        // 日平均气温 ( 单位：摄氏度 )
        averageHandle(dayValueTab,"V12001_701","V12001",hourValueTab,size);
        // 日平均本站气压 ( 单位：百帕 )
        averageHandle(dayValueTab,"V10004_701","V10004",hourValueTab,size);
        // 日最小相对湿度 ( 单位：% )、日最小相对湿度出现时间 ( 格式：时分 )
        extremeHandle(dayValueTab,"V13007","V13007","V13007_052","V13007_052",dayValueTab,"s");
        //日平均相对湿度
        averageHandle(dayValueTab,"V13003_701","V13003",hourValueTab,size);
        // 日平均总云量 ( 单位：% )
        averageHandle(dayValueTab, "V20010_701", "V20010", hourValueTab, size);
        // 日平均低云量 ( 单位：% )
        averageHandle(dayValueTab, "V20051_701", "V20051", hourValueTab, size);
        // 日平均水汽压 ( 单位：百帕 )
        averageHandle(dayValueTab, "V13004_701", "V13004", hourValueTab, size);
        //日最大水汽压
        ...
        //日平均露点温度
        averageHandle(dayValueTab, "V12003_avg", "V12003", hourValueTab, size);
        //日平均湿球温度
        averageHandle(dayValueTab, "balltemp_avg", "balltemp", hourValueTab, size);
        //日平均湿球温度(24小时)
