package cma.cimiss2.dpc.indb.cawn.dc_chn_dst;

import cma.cimiss2.dpc.decoder.bean.ParseResult;
import cma.cimiss2.dpc.decoder.bean.ReportError;
import cma.cimiss2.dpc.decoder.bean.cawn.ChnDst;
import cma.cimiss2.dpc.decoder.cawn.DecodeChnDst;
import cma.cimiss2.dpc.decoder.tools.FileEncodeUtil;
import cma.cimiss2.dpc.decoder.tools.enumeration.Action;
import cma.cimiss2.dpc.indb.cawn.dc_chn_dst.Service.ChnDstService;
...
public class ChnDstThread implements Runnable {
    ...
    public ChnDstThread(String topic) {
        this.topic = topic;
    }
    ...
    public void run() {
        try {
            if (topic != null && !("".equals(topic))) {
                ...
                String pathname = "E:\\冬奥项目\\资料样例\\结构化数据\\1.3.2  沙尘（暴）数据集(v1.0)\\SURF_CLI_CHN_DAY-WEP-DSS-195401.TXT";
    ...
    private Action processMsg(File file, String fileCode, Date recv_time, StringBuffer loggerBuffer, String filename) {
        ...
        // 数据解码返回解码结果集
        DecodeChnDst decodeChnDst = new DecodeChnDst();
        parseResult = decodeChnDst.decode(file,filename);
        // 判断是否解码成功
        if (parseResult.isSuccess()) {
            ...
            if (StartConfig.getDatabaseType() == 1 || StartConfig.getDatabaseType() == 0) {
                action = ChnDstService.processSuccessReport(parseResult, recv_time, topic, loggerBuffer, filename);
            } else {
                return Action.RETRY;
            }
