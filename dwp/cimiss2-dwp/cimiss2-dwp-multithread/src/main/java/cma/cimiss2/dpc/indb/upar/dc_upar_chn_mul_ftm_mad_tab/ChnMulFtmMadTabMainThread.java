package cma.cimiss2.dpc.indb.upar.dc_upar_chn_mul_ftm_mad_tab;


import cma.cimiss2.dpc.decoder.fileDecode.util.FileHadleUtil;
import cma.cimiss2.dpc.decoder.tools.common.StatDi;
import cma.cimiss2.dpc.indb.upar.dc_upar_dig_nm_stl_ftm_tab.DigNmStlFtmTabSubThread;
...
/**
 * @author ：YCK
 * @date ：Created in 2019/10/23 0023 9:08
 * @description：中国高空规定等压面定时值数据集(v2.0)数据表
 * @modified By：
 * @version: 1.0$
 */
public class ChnMulFtmMadTabMainThread {
//    private static String configPropertiesFile = "D:\\解码文件\\J.0011.0001.S001_config.properties";
//
//    /**
//     * create by zxj
...
//                excutor.execute(new ChnMulFtmMadTabSubThread(topic));
//            }
...
public static void start1(){
        ...
        String ctS = StartConfig.ctsCode("R017_cts_code");
        String sodSs[] = StartConfig.sodCodes("R017_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R017_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new ChnMulFtmMadTabSubThread(diQueues));
            }
        }
    }
    ...
    private static ChnMulFtmMadTabSubThread chnMulFtmMadTabSubThread=  new ChnMulFtmMadTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R017_ChnMuFtmMad_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R017_ChnMuFtmMad_task_time"));

    public static void start(){
        //初始化文件时间
        FileDataTimeUtil.loadFileDataProperties();
        new Timer("ChnMuFtmMad" ).schedule(new TimerTask() {
            @Override
            public void run() {
                List<String> list=new ArrayList<>();
                String lastTime = FileDataTimeUtil.getProperties().get("R017_ChnMuFtmMad_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    chnMulFtmMadTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R017_ChnMuFtmMad_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
