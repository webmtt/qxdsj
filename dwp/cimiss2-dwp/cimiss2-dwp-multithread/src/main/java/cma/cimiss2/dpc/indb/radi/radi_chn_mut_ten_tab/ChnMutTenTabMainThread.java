package cma.cimiss2.dpc.indb.radi.radi_chn_mut_ten_tab;

import cma.cimiss2.dpc.decoder.fileDecode.util.FileHadleUtil;
import cma.cimiss2.dpc.decoder.tools.common.StatDi;
import cma.cimiss2.dpc.indb.radi.radi_chn_mut_pen_tab.ChnMutPenTabSubThread;
...
public class ChnMutTenTabMainThread {
//    private static String configPropertiesFile = "D:\\解码文件\\MB.0011.0001.S001_config.properties";
//
//    /**
//     * create by zxj
...
//                excutor.execute(new ChnMutTenTabSubThread(topic));
//            }
...
public static void start1() {
        ...
        String ctS = StartConfig.ctsCode("R006_cts_code");
        String sodSs[] = StartConfig.sodCodes("R006_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R006_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new ChnMutTenTabSubThread(diQueues));
            }
        }
    }
    public static void main(String[] args) {
    ...
    private static ChnMutTenTabSubThread chnMutTenTabSubThread=  new ChnMutTenTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R006_ChnMutTen_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R006_ChnMutTen_task_time"));

    public static void start(){
        //初始化文件时间
        FileDataTimeUtil.loadFileDataProperties();
        new Timer("ChnMutTen" ).schedule(new TimerTask() {
            @Override
            public void run() {
                List<String> list=new ArrayList<>();
                String lastTime = FileDataTimeUtil.getProperties().get("R006_ChnMutTen_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    chnMutTenTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R006_ChnMutTen_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
