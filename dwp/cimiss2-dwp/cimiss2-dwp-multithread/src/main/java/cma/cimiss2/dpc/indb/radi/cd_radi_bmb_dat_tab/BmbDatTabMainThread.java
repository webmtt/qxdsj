...
import cma.cimiss2.dpc.indb.upar.dc_upar_chn_mul_ftm_wew_tab.ChnMulFtmWewTabSubThread;
 ...
 * @description：巴盟自建辐射数据
 * @modified By：
 * @version: 1.0$
 */
public class BmbDatTabMainThread {
//    private static String configPropertiesFile = "D:\\G.0011.0001.S001_config.properties";
//
//    /**
//     * create by zxj
...
//                excutor.execute(new BmbDatTabSubThread(topic));
//            }
...
public static void start1(){
        ...
        String ctS = StartConfig.ctsCode("R016_cts_code");
        String sodSs[] = StartConfig.sodCodes("R016_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R016_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new BmbDatTabSubThread(diQueues));
            }
        }
    }
    ...
    private static BmbDatTabSubThread bmbDatTabSubThread=  new BmbDatTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R016_RadiBmbDat_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R016_RadiBmbDat_task_time"));
    ...
    public static void start(){
        ...
        new Timer("RadiBmbDat" ).schedule(new TimerTask() {
            ...
            public void run() {
                ...
                String lastTime = FileDataTimeUtil.getProperties().get("R016_RadiBmbDat_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    bmbDatTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R016_RadiBmbDat_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
