...
import cma.cimiss2.dpc.indb.agme.dc_grass_nutrition.GrassNutritionThread;
 ...
 */
public class ChnSummatyMainThread {
    ...
    public static void start(){
        ...
        String ctS = StartConfig.ctsCode("S014_cts_code");
        String sodSs[] = StartConfig.sodCodes("S014_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("S014_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new ChnSummatyThread(diQueues));
            }
        }
    }
}
