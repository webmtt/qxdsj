...
import cma.cimiss2.dpc.indb.radi.dc_radi_chn_mut_mten_tab.ChnMutMtenTabSubThread;
 ...
 * @description：中国气象辐射基本要素累年月值数据集(v2.0)数据表
 * @modified By：
 * @version: 1.0$
 */
public class ChnMutMmonTabMainThread {
//    private static String configPropertiesFile = "D:\\P.0011.0001.S001_config.properties";
//
//    /**
//     * create by zxj
...
//                excutor.execute(new ChnMutMmonTabSubThread(topic));
//            }
...
public static void main(String[] args) throws Exception {
    ...
    public static void start1(){
        ...
        String ctS = StartConfig.ctsCode("R015_cts_code");
        String sodSs[] = StartConfig.sodCodes("R015_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R015_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new ChnMutMmonTabSubThread(diQueues));
            }
        }
    }
    private static BlockingQueue<StatDi> diQueues = new LinkedBlockingQueue<StatDi>();
    private static ChnMutMmonTabSubThread chnMutMmonTabSubThread=  new ChnMutMmonTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R015_ChnMulMmon_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R015_ChnMulMmon_task_time"));

    public static void start(){
        //初始化文件时间
        FileDataTimeUtil.loadFileDataProperties();
        new Timer("ChnMulMmon" ).schedule(new TimerTask() {
            @Override
            public void run() {
                List<String> list=new ArrayList<>();
                String lastTime = FileDataTimeUtil.getProperties().get("R015_ChnMulMmon_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    chnMutMmonTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R015_ChnMulMmon_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
