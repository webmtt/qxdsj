...
import cma.cimiss2.dpc.indb.radi.radi_chn_mut_yer_tab.ChnMutYerTabSubThread;
 ...
 * @version: 1.0$
 */
public class HourValueTabMainThread {
...
public static void start1() {
        ...
        String ctS = StartConfig.ctsCode("R022_cts_code");
        String sodSs[] = StartConfig.sodCodes("R022_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R022_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new HourValueTabSubThread(diQueues));
            }
        }
    }
    ...
    private static HourValueTabSubThread hourValueTabSubThread=  new HourValueTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R022_HourValueTab_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R022_HourValueTab_task_time"));

    public static void start(){
        //初始化文件时间
        FileDataTimeUtil.loadFileDataProperties();
        new Timer("HourValueTab" ).schedule(new TimerTask() {
            @Override
            public void run() {
                List<String> list=new ArrayList<>();
                String lastTime = FileDataTimeUtil.getProperties().get("R022_HourValueTab_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    hourValueTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R022_HourValueTab_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
