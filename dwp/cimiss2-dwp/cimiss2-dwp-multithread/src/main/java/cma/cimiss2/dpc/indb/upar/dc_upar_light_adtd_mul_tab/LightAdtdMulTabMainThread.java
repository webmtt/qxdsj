...
/**
 * @author ：YCK
 * @date ：Created in 2019/10/12 0012 15:17
 * @description：闪电定位数据表
 * @modified By：
 * @version: 1.0$
 */
public class LightAdtdMulTabMainThread {
//    private static String configPropertiesFile = "D:\\B.0016.0003.S001_config.properties";
//
//    /**
//     * create by zxj
...
//                excutor.execute(new LightAdtdMulTabSubThread(topic));
//            }
...
public static void start1(){
        ...
        String ctS = StartConfig.ctsCode("R020_cts_code");
        String sodSs[] = StartConfig.sodCodes("R020_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R020_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new LightAdtdMulTabSubThread(diQueues));
            }
        }
    }
    ...
    private static LightAdtdMulTabSubThread lightAdtdMulTabSubThread=  new LightAdtdMulTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R20_lightAdtMul_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R20_lightAdtMul_task_time"));

    public static void start(){
        //初始化文件时间
        FileDataTimeUtil.loadFileDataProperties();
        new Timer("LightAdtMul" ).schedule(new TimerTask() {
            @Override
            public void run() {
                List<String> list=new ArrayList<>();
                String lastTime = FileDataTimeUtil.getProperties().get("R20_lightAdtMul_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    lightAdtdMulTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R20_lightAdtMul_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
