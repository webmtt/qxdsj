...
/**
 * 天然牧草营养成分
 * Created by zxj on 2019/10/30.
 */
public class GrassNutritionMainThread {
    ...
    public static void start(){
        ...
        String ctS = StartConfig.ctsCode("S023_cts_code");
        String sodSs[] = StartConfig.sodCodes("S023_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("S023_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new GrassNutritionThread(diQueues));
            }
        }
    }
}
