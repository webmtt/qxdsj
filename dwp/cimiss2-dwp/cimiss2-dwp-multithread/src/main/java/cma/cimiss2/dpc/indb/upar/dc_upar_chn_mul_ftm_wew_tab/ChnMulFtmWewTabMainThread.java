package cma.cimiss2.dpc.indb.upar.dc_upar_chn_mul_ftm_wew_tab;


import cma.cimiss2.dpc.decoder.fileDecode.util.FileHadleUtil;
import cma.cimiss2.dpc.decoder.tools.common.StatDi;
import cma.cimiss2.dpc.indb.upar.dc_upar_chn_mul_ftm_sig_tab.ChnMulFtmSigTabSubThread;
 ...
 * @description：中国高空规定高度层风定时值数据集(v1.0)数据表
 * @modified By：
 * @version: 1.0$
 */
public class ChnMulFtmWewTabMainThread {
//    private static String configPropertiesFile = "D:\\解码文件\\Y.0011.0001.S001_config.properties";
//
//    /**
//     * create by zxj
...
//                excutor.execute(new ChnMulFtmWewTabSubThread(topic));
//            }
...
public static void start1(){
        ...
        String ctS = StartConfig.ctsCode("R021_cts_code");
        String sodSs[] = StartConfig.sodCodes("R021_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R021_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new ChnMulFtmWewTabSubThread(diQueues));
            }
        }
    }
    ...
    private static ChnMulFtmWewTabSubThread chnMulFtmWewTabSubThread=  new ChnMulFtmWewTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R021_ChnMuFtmWew_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R021_ChnMuFtmWew_task_time"));

    public static void start(){
        //初始化文件时间
        FileDataTimeUtil.loadFileDataProperties();
        new Timer("ChnMuFtmWew" ).schedule(new TimerTask() {
            @Override
            public void run() {
                List<String> list=new ArrayList<>();
                String lastTime = FileDataTimeUtil.getProperties().get("R021_ChnMuFtmWew_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    chnMulFtmWewTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R021_ChnMuFtmWew_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
