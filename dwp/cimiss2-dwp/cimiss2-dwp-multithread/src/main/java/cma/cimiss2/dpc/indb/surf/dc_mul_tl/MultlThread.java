package cma.cimiss2.dpc.indb.surf.dc_mul_tl;

import cma.cimiss2.dpc.decoder.bean.ParseResult;
import cma.cimiss2.dpc.decoder.bean.ReportError;
import cma.cimiss2.dpc.decoder.bean.surf.Muldy;
import cma.cimiss2.dpc.decoder.surf.DecodeMultl;
import cma.cimiss2.dpc.decoder.tools.FileEncodeUtil;
import cma.cimiss2.dpc.decoder.tools.enumeration.Action;
import cma.cimiss2.dpc.indb.surf.dc_mul_tl.Service.MultlService;
...
public class MultlThread implements Runnable{
    ...
    public MultlThread(String topic) {
        this.topic = topic;
    }
    ...
    private Action processMsg(byte[] data, String fileCode, Date recv_time, StringBuffer loggerBuffer, String filename) {
        ...
        // 数据解码返回解码结果集
        DecodeMultl decodeMultl = new DecodeMultl();
        parseResult = decodeMultl.decode(data,fileCode);
        // 判断是否解码成功
        if (parseResult.isSuccess()) {
            ...
            if (StartConfig.getDatabaseType() == 1 || StartConfig.getDatabaseType() == 0) {
                action = MultlService.processSuccessReport(parseResult, recv_time, topic, loggerBuffer, filename);
            } else {
                return Action.RETRY;
            }
