package cma.cimiss2.dpc.indb.radi.dc_radi_chn_mut_mten_tab;

import cma.cimiss2.dpc.decoder.fileDecode.util.FileHadleUtil;
import cma.cimiss2.dpc.decoder.tools.common.StatDi;
import cma.cimiss2.dpc.indb.radi.dc_radi_chn_mut_mpen_tab.ChnMutMpenTabSubThread;
...
/**
 * @author ：YCK
 * @date ：Created in 2019/10/28 0028 16:37
 * @description：中国气象辐射基本要素累月旬值数据集(v2.0)数据表
 * @modified By：
 * @version: 1.0$
 */
public class ChnMutMtenTabMainThread {
//    private static String configPropertiesFile = "D:\\W.0011.0001.S001_config.properties";
//
//    /**
//     * create by zxj
...
//                excutor.execute(new ChnMutMtenTabSubThread(topic));
//            }
...
public static void start1(){
        ...
        String ctS = StartConfig.ctsCode("R013_cts_code");
        String sodSs[] = StartConfig.sodCodes("R013_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R013_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new ChnMutMtenTabSubThread(diQueues));
            }
        }
    }
    ...
    private static ChnMutMtenTabSubThread chnMutMtenTabSubThread=  new ChnMutMtenTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R013_ChnMulMten_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R013_ChnMulMten_task_time"));

    public static void start(){
        //初始化文件时间
        FileDataTimeUtil.loadFileDataProperties();
        new Timer("ChnMulMten" ).schedule(new TimerTask() {
            @Override
            public void run() {
                List<String> list=new ArrayList<>();
                String lastTime = FileDataTimeUtil.getProperties().get("R013_ChnMulMten_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    chnMutMtenTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R013_ChnMulMten_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
