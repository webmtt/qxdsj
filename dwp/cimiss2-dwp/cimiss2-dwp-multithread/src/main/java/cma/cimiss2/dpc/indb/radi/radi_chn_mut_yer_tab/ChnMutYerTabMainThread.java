package cma.cimiss2.dpc.indb.radi.radi_chn_mut_yer_tab;

import cma.cimiss2.dpc.decoder.fileDecode.util.FileHadleUtil;
import cma.cimiss2.dpc.decoder.tools.common.StatDi;
import cma.cimiss2.dpc.indb.radi.radi_chn_mut_ten_tab.ChnMutTenTabSubThread;
...
public class ChnMutYerTabMainThread {
//    private static String configPropertiesFile = "D:\\解码文件\\MA.0011.0001.S001_config.properties";
//
//    /**
//     * create by zxj
...
//                excutor.execute(new ChnMutYerTabSubThread(topic));
//            }
...
public static void start1(){
        ...
        String ctS = StartConfig.ctsCode("R007_cts_code");
        String sodSs[] = StartConfig.sodCodes("R007_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("R007_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new ChnMutYerTabSubThread(diQueues));
            }
        }
    }
    ...
    private static ChnMutYerTabSubThread chnMutYerTabSubThread=  new ChnMutYerTabSubThread(diQueues);
    private static String path=StartConfig.ctsCode("R007_ChnMutYer_file_path");
    private static int taskTime=Integer.parseInt(StartConfig.ctsCode("R007_ChnMutYer_task_time"));

    public static void start(){
        //初始化文件时间
        FileDataTimeUtil.loadFileDataProperties();
        new Timer("ChnMutYer" ).schedule(new TimerTask() {
            @Override
            public void run() {
                List<String> list=new ArrayList<>();
                String lastTime = FileDataTimeUtil.getProperties().get("R007_ChnMutYer_lastTime").toString().trim();
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    chnMutYerTabSubThread.processMsg(filePath, new Date(), loggerBuffer);
...
//                    FileUtils.deleteQuietly(file);
                ...
                if(maxFileTime!=0) {
                    //更新文件时间
                    FileDataTimeUtil.updateFileData("R007_ChnMutYer_lastTime", maxFileTime + "");
                }
            }
        },0,taskTime);
    }
