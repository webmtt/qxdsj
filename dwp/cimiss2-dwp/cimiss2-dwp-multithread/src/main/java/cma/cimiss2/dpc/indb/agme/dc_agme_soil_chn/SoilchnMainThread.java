...
/**
 * 生态土壤墒情报
 * Created by zxj on 2019/11/04.
 */
public class SoilchnMainThread {
    ...
    public static void start(){
        ...
        String ctS = StartConfig.ctsCode("S022_cts_code");
        String sodSs[] = StartConfig.sodCodes("S022_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("S022_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new SoilchnThread(diQueues));
            }
        }
    }
}
