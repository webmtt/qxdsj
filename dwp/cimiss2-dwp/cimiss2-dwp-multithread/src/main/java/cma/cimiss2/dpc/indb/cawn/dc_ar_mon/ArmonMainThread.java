...
import cma.cimiss2.dpc.indb.cawn.dc_ar_day.ArdayThread;
...
public class ArmonMainThread {
    ...
    public static void start(){
        ...
        String ctS = StartConfig.ctsCode("S034_cts_code");
        String sodSs[] = StartConfig.sodCodes("S034_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("S034_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new ArmonThread(diQueues));
            }
        }
    }
}
