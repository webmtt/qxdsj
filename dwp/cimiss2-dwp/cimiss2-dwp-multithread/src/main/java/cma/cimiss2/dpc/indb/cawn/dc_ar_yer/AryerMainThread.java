...
public class AryerMainThread {
    ...
    public static void start(){
        ...
        String ctS = StartConfig.ctsCode("S036_cts_code");
        String sodSs[] = StartConfig.sodCodes("S036_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("S036_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new AryerThread(diQueues));
            }
        }
    }
}
