package cma.cimiss2.dpc.indb.surf.dc_chn_summaty.Service;
...
public class ChnSummatyService {
    ...
    private static void insert_db(List<Chnmrbc> parseResult, Connection connection, Date recv_time, StringBuffer loggerBuffer, String filename, String cts_code) {
        ...
        try {
            String sql = " INSERT INTO " + StartConfig.valueTable("S014_value_table_name") + "("
                    + "D_RECORD_ID, V01301,V01300, SUMMATY_MONTH, SUMMARY_EVENTID, SUMMARY_EVENT, SUMMARY_EVENT_STOP_TIME,EVENT_TYPE_IDENTIFICATION)"
                    + "VALUES (?,?,?,?,?,?,?,?)";
            ...
            for (Chnmrbc chnmrbc : parseResult) {
                ...
                try {
                    ...
                    ps.setString(ii++, chnmrbc.getSummatyMonth());
                    ps.setString(ii++, chnmrbc.getSummaryEventid());
                    ps.setString(ii++, chnmrbc.getSummaryEvent());
                    ps.setString(ii++, chnmrbc.getSummatyMonth());
                    ps.setString(ii++, chnmrbc.getEventTypeIdentification1());
                } catch (Exception e) {
                    e.printStackTrace();
