...
public class DigchnMainThread {
    ...
    public static void start(){
        ...
        String ctS = StartConfig.ctsCode("S009_cts_code");
        String sodSs[] = StartConfig.sodCodes("S009_sod_code");
        String reportSods[] = StartConfig.reportSodCodes();
        String valuetables[] = StartConfig.valueTables("S009_value_table_name");
...
//            codeMap.setReport_sod_code(reportSods[i]);
        ...
        if(StartConfig.fileLoop() == 0) {
            for (int i = 0; i < threadCount; i++) {
                executor.execute(new DigchnThread(diQueues));
            }
        }
    }
}
