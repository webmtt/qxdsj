...
public class KettleInit {
    ...
    @Value("${kettle.plugin.json}")
    private static String pluginFolder4Json;

    /**
     * 初始化Kettle环境。 此方法将尝试配置 简单的JNDI， 通过简单地调用init(true)。
     *
     * @throws KettleException 在初始化过程中发生的任何错误都将抛出 KettleException。
...
//		init(true);
     ...
     * 启动监听程序等
     *
     * @param simpleJndi ， 真正简单的JNDI配置，否则返回false
     * @throws KettleException 在初始化过程中发生的任何错误都将抛出 KettleException。
     */
    public static void init(boolean simpleJndi) throws KettleException {
        if (initialized == null) {
            ...
            PluginRegistry.init();
            // 添加插件路径
			StepPluginType.getInstance().getPluginFolders().add(new PluginFolder(Constant.KETTLE_PLUGIN_JSON,false,true));
//			StepPluginType.getInstance().getPluginFolders().add(new PluginFolder("D:\\workspace\\ywj\\etl\\test\\kettle-build\\pdi-ce-8.2.0.0-342-20200306\\data-integration\\plugins\\kettle-json-plugin",false,true));
//			StepPluginType.getInstance().getPluginFolders().add(new PluginFolder("/export/data-integration/plugins/kettle-json-plugin",false,true));
            // 初始化读取的变量列表。
     ...
     * 提醒所有生命周期的插件水壶环境。 初始化。
     *
     * @throws KettleException 当一个生命周期侦听器抛出一个异常
     */
     ...
     * 加载插件注册表。
     *
     * @throws KettlePluginException 加载插件如果遇到任何错误返回KettlePluginException
     */
     ...
     * 初始化环境变量。
     *
     * @throws KettleException 初始化如果遇到任何错误返回KettleException
     */
